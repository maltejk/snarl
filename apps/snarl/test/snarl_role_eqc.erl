...
setup() ->
    ...
    meck:expect(snarl_user, revoke_prefix, fun(_, _) -> ok end),
    meck:expect(snarl_user, leave, fun(_, _) -> ok end),
    meck:expect(snarl_user, list, fun() -> {ok, []} end),
    meck:expect(snarl_user, get_, fun(_) -> {ok, dummy} end),
