        ...
        {ok, UserObj} ->
            {ok, lists:foldl(
                   fun(Role, Permissions) ->
                           case snarl_role:get_(Realm, Role) of
                               {ok, RoleObj} ->
...
-spec get_(Realm::binary(), User::fifo:user_id()) ->
                  not_found |
                  {error, timeout} |
                  {ok, User::#?USER{}}.
get_(Realm, User) ->
...
join_org(Realm, User, Org) ->
    case snarl_org:get_(Realm, Org) of
        {ok, _} ->
